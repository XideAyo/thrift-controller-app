...
$(window).resize(function() {
  ...
  if ($('.full-screen-map').length == 0 && $('.bd-docs').length == 0) {
    ...
    if ($navbar.hasClass('bg-white') && $(window).width() > 991) {
      $navbar.removeClass('bg-white').addClass('navbar-trans');
    } else if ($navbar.hasClass('navbar-trans') && $(window).width() < 991 && isExpanded != "false") {
      $navbar.addClass('bg-white').removeClass('navbar-trans');
    }
  }
});
